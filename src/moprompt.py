...
from models import PromptBERT, RawBert
from dataset import PromptDataset


def train(supervised, epochs, dataloader, dev_dataloader, model, raw_bert, optimizer, save_path, lambda_):
    model.to(config.device)
    raw_bert.to(config.device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch, data in tqdm(enumerate(dataloader), total=total, postfix={'epoch': epoch}):
            ...
            guide_a = raw_bert(*sen_a[2])
            guide_b = raw_bert(*sen_b[2])

            loss = utils.mo_prompt_compute_loss(out_a=out_a, out_b=out_b, guide_a=guide_a, guide_b=guide_b, lambda_=lambda_)
            ...
            for query_params, key_params in zip(model.encoder.parameters(), raw_bert.bert.parameters()):
                key_params.data.copy_(config.momentum * key_params + (1 - config.momentum) * query_params)
...
def moprompt(model_save_folder, epochs=config.epochs,
             file_paths=config.file_paths, check_points=config.check_points_main, pool_ways=config.pool_ways,
             templates=config.prompt_template, denoise=config.denoise, momentum=True,
             attention_dropout=config.attention_dropout, hidden_dropout=config.hidden_dropout, lr=config.learning_rate, lambda_=config.mo_lambda_):
    # 创建 prompt_baseline 训练模型保存文件夹
    model_save_path = os.path.join(model_save_folder, "mo_prompt")
    ...
    for data_name, files in file_paths.items():
        ...
        for check_point in check_points:
            ...
            for template in templates:
                ...
                train_prompt = utils.build_prompt_inputs(data=train_data, template=template, tokenizer=tokenizer, momentum=momentum)
                dev_prompt = utils.build_prompt_inputs(data=dev_data, template=template, tokenizer=tokenizer, momentum=momentum)
                test_prompt = utils.build_prompt_inputs(data=test_data, template=template, tokenizer=tokenizer, momentum=momentum)

                # 数据集的构造也不用改变，只是使用数据集的方式不一样
                ...
                for pool_way in pool_ways:
                    ...
                    raw = copy.deepcopy(encoder)
                    raw_bert = RawBert(bert=raw, pool_way=pool_way)
                    raw_bert.bert.resize_token_embeddings(len(tokenizer))

                    # prompt Bert 本身的构造还是不变的
